...
fn ilerp(a: vec2f, b: vec2f, v: vec2f) -> vec2f {
...
fn cbrtv(x: vec2f) -> vec2f {
    return vec2f(cbrt(x.x), cbrt(x.y));
}
...
fn bezier(P1: vec2f, P2: vec2f, P3: vec2f, uv: vec2f, thickness: f32) -> f32 {
    ...
    // let AA = mix(P1, P2, timer);
    // let BB = mix(P2, P3, timer);
    // let CC = mix(AA, BB, timer);

    // let AA = mix(P1, P2, result);
    // let BB = mix(P2, P3, result);
    // let CC = mix(AA, BB, result);

    // let der = 2 * (A + B * timer);
    // let tangent = vec2f(der);
    // let tangent = BB - AA;
    // let normal = normalize(vec2f(-tangent.y, tangent.x)) * 0.1;
    ...
    // return step(result, thickness) + line(P1, refract(P1, P2, 1), uv, 0.01);
    // return step(result, thickness) * 0.1 + line(
    //     CC - normal,
    //     CC + normal,
    //     uv,
    //     0.01
    // );
    // return smoothstep(0.01, 0, result);
...
fn bezier2(p1: vec2f, c1: vec2f, p2: vec2f, uv: vec2f, thickness: f32) -> f32 {
    ...
    let ah = p1 * (fx * fx) + c1 * (2 * t * fx) + p2 * t * t;

    let point_deriv = 2 * (1 - t) * (c1 - p1) + 2 * t * (p2 - c1);

    // dot(point_deriv, point - uv) = 0
...
fn fragment_main(@builtin(position) in: vec4f) -> @location(0) vec4f {
    ...
    for (var i = 1; i <= amount; i++) {
        ...
        let out = bezier(
            vec2f(
                ...
                flip * cos(rate * 0.5) * 0.5,
                flip * sin(rate * 0.5) * 0.5,
            ),
            vec2f(
                sin(theta + rate) * 0.5,
                cos(theta + rate) * 0.5,
            ...
            vec2f(
                cos(theta + rate * 0.25) * 0.5,
                sin(theta + rate * 0.25) * 0.5,
            ),
            uv,
            0.0015
        );
    ...
    // color += (sin(length(uv - c) * 200) * 0.5 + 0.5) * 0.5;
    // color += (sin(length(uv) * 200 + time * 0.01) * 0.5 + 0.5) * 0.5;
    ...
    //     sin(time * 0.001)
    // );
    // color += 1 - (dot(uv - p1, p4 - p1) / dot(p4 - p1, p4 - p1) + length(uv));
    // color += 1 - (dot(uv * 10, -p4 * 2) + length(uv * 10));
